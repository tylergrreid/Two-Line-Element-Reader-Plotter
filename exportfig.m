...
%
%   EXPORTFIG(FILENAME) writes the current figure to FILENAME.
%
%   EXPORTFIG(H, FILENAME) writes the figure H to FILENAME.  H is a figure
%   handle and FILENAME is a string that specifies the name of the output
%   file.
%
%   EXPORTFIG(...,PARAM1,VAL1,PARAM2,VAL2,...) specifies parameters that
%   control various characteristics of the output file.
%
%   Format Paramter:
%     'Format'  one of the strings 'pdf','eps','eps2','jpeg','png','preview'
%          specifies the output format. Defaults to 'pdf'. The output
%          format 'preview' does not generate an output file but instead
%          creates a new figure window with a preview of the exported
%          figure. In this case the FILENAME parameter is ignored. The file
%          format can be specified using the file extension in the
%          filename. For example exportfig('filename.pdf') exports as pdf,
%          exportfig('filename.eps') exports as eps, etc.
%
...
%          specifies the width in the figure's PaperUnits. Default = 3.67
%          inches, an appropriate width for two column journal articles.
...
%                 using the CMYK color space. (default).
%         'rgb'   Modification by Todd Karin to add in rgb colorscheme.
%                 This is more accurate in certain situations.
...
%     
%     The default color setting is 'rgb'.
%
%
%   Font Parameters:
%     'FontMode'     one of the strings 'scaled', 'fixed' (default)
...
% Modification by Todd Karin. Read option for pdf output.
[fileNamePrefix, fileFormat] = splitStringAtDot(filename);
fileNameEps = [fileNamePrefix '.eps'];
fileNamePdf = [fileNamePrefix '.pdf'];
...
% End mod.
format = fileFormat;
if strcmp(format,'pdf'), format = 'eps'; end
preview = 'none';
width = 3.67;
height = -1;
color = 'rgb';
...
for k = 1:2:length(paramPairs)
  ...
  switch (param)
   case 'format'
    format = value;
    if (~strcmp(format,{'pdf','eps','eps2','jpeg','png','preview'}))
        error(['Format must be ''pdf'',''eps'', ''eps2'', ''jpeg'', ''png'' or' ...
            ' ''preview''.']);
   ...
   case 'color'
    ...
    if (~strcmp(color,{'bw','gray','cmyk','rgb'}))
      error('Color must be ''bw'', ''gray'' or ''cmyk'' or  ''rgb''.');
    end
   case 'fontmode'
   ...
   case 'delta'
    delta = value;
    if ~ (length(delta)==4)
        error('Length of delta must be 4')
    end
   otherwise
...
if sum(strncmp(format,{'eps','pdf'},3)) && ~strcmp(preview,'none')
    args = {args{:}, ['-' preview]};
end

hadError = 0;

% Process size parameters, cropping
...
set(H, 'PaperPosition', [0 0 width height]+delta); % Todd Karin: modified to fix bounding box if slightly wrong.
...
if strcmp(fontencoding,'adobe') & sum(strncmp(format,{'eps','pdf'},3))
args = {args{:}, '-adobecset'};
end
...
% Modification by Todd Karin to export to pdf.



switch fileFormat
    case 'pdf'
        print(H, fileNameEps, args{:});
        eps2pdf(fileNameEps,fileNamePdf);
        delete(fileNameEps);
    otherwise
...
function [start, ext] = splitStringAtDot(s)
%splitStringAtDot - retruns the parts of the string before and after a '.'
%
% Example
%
%   [a,b] = splitStringAtDot('fname.ext')
%
%   a =
%       fname
%   b =
